    ...
    fun deleteFromEditor(
        ...
        scope.launch {
            try {
                eventWriteMutex.withLock {
                    if (storedEvent != null) {
                        // The deletion is queued as the record's current local revision, which
                        // supersedes any queued draft for the same UID. It must NOT be discarded
                        // afterwards, or the tombstone would never reach the server.
