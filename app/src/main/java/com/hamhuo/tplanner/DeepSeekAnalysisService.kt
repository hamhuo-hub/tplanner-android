...
// 手机直连 DeepSeek API（deepseek-v4-pro 思考模式），不走树莓派。
//
...
class DeepSeekAnalysisService(private val apiKey: String) {
    ...
    // ── 会话记忆：同一轮"理一理"内多步共享上下文 ──────────────────────
    // 思考模式下 deepseek-v4-pro 会生成 reasoning_content，但不带工具调用时
    // API 会忽略传入的 reasoning_content，只取 content。所以多轮拼接只需要
    // 累积 role/content 的 user/assistant 消息即可。
    ...
    //
    // 思考模式规则（无工具调用）：
    // - reasoning_content 由模型生成，帮助提高回答质量
    // - 后续轮次中，API 会忽略传入的 reasoning_content，只需传 content
    // - 直接 append response.choices[0].message 即可
    private fun callDeepSeekWithHistory(currentUserMessage: String): String {
        ...
        conn.readTimeout = 120000  // 思考模式需要更长时间
        conn.doOutput = true
        ...
        try {
            ...
            if (reasoningLen > 0) {
                android.util.Log.d("TplannerDS", "reasoning_content length=$reasoningLen chars")
            }
    ...
    companion object {
        ...
        // 思考模式模型：deepseek-v4-pro 会在回答前生成思维链（reasoning_content）
        // 从而提升复杂推理类任务的质量（如判断文字中隐含的困惑、提炼追问切入点）
