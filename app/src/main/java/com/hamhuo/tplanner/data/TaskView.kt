...
sealed class TaskView(val key: String) {
        ...
        when (this) {
            ...
            Today -> {
                ...
                val overdue = items.filter { item ->
                    item.deletedAt == 0L && item.type == "task" && item.scheduled && !item.completed &&
    ...
    /** Recurring occurrences already arrive expanded from the canonical documents. */
    fun listItems(items: List<ScheduleItem>, date: LocalDate = appToday()): List<ScheduleItem> =
        collapseRecurringTaskSeries(filter(items, date))
