...
ANDROID_SOURCE = ROOT / "shared/src/main/kotlin/com/hamhuo/tplanner/designsystem/TPlannerDesignTokens.kt"
ANDROID_COLORS = ROOT / "shared/src/main/res/values/tplanner_light_colors.xml"
ANDROID_CSS = ROOT / "shared/src/main/assets/tplanner_light.css"
ANDROID_BEGIN = "// BEGIN GENERATED LIGHT TOKENS"
ANDROID_END = "// END GENERATED LIGHT TOKENS"
SUPPORTED_TYPES = {
...
def android_outputs(tokens: dict[str, Token], resolved: dict[str, Any], css: str) -> dict[Path, str]:
    """Opt-in integration: preserve handwritten/legacy tokens around one generated block."""
    existing = ANDROID_SOURCE.read_text(encoding="utf-8-sig")
    generated = render_kotlin(tokens, resolved).split("object TPlannerLightTokens {", 1)[1]
    block = ANDROID_BEGIN + "\nobject TPlannerLightTokens {" + generated + ANDROID_END + "\n"
    if ANDROID_BEGIN in existing or ANDROID_END in existing:
        if existing.count(ANDROID_BEGIN) != 1 or existing.count(ANDROID_END) != 1:
            fail("Android generated block markers must each occur exactly once")
        start, end = existing.index(ANDROID_BEGIN), existing.index(ANDROID_END)
        if end <= start:
            fail("Android generated block markers are out of order")
        source = existing[:start] + block + existing[end + len(ANDROID_END):].lstrip("\n")
    else:
        source = existing.rstrip() + "\n\n" + block
    xml = ['<?xml version="1.0" encoding="utf-8"?>',
           '<!-- Generated by scripts/generate-design-tokens.py (Android export). Do not edit. -->',
           '<resources>']
    for name, token in sorted(tokens.items()):
        if token.path[:2] == ("semantic", "color"):
            resource = "tp_light_" + "_".join(part.lower() for part in words(token.path[-1]))
            red, green, blue, alpha = color_channels(resolved[name])
            xml.append(f'    <color name="{resource}">#{alpha:02X}{red:02X}{green:02X}{blue:02X}</color>')
    xml.extend(['</resources>', ''])
    return {ANDROID_SOURCE: source, ANDROID_COLORS: "\n".join(xml), ANDROID_CSS: css}
...
def main() -> int:
    ...
    parser.add_argument("--android", action="store_true", help="Also update/check the canonical Android block and shared XML colors")
    ...
    try:
        ...
        outputs = {args.output / name: content for name, content in render_outputs(tokens, resolved, report).items()}
        if args.android:
            if args.source.resolve() != DEFAULT_SOURCE.resolve():
                fail("--android requires the canonical token source")
            outputs.update(android_outputs(tokens, resolved, outputs[args.output / "tplanner-light.css"]))
        if args.check:
            stale = [str(path) for path, content in outputs.items()
                     if not path.is_file() or
                     path.read_text(encoding="utf-8-sig") != content]
            if stale:
