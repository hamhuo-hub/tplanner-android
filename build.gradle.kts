...
// ── 版本管理：git tag 是唯一版本源 ─────────────────────────────────────────
// 发版 = 打 tag `v6.0.2`（scripts/release.ps1 负责校验与打 tag）。
// versionName / versionCode 在构建时由 git 推导，代码里不再手写版本号：
//   - HEAD 恰好打在 v* tag 上 → 正式版：versionName = 6.0.2
...
// 发版 tag 认 v*（新约定）与 PUKEKO_*（历史遗留，仅作兜底），屏蔽 TUI_/mobile_ 等杂 tag。
// 前缀式解析，兼容 describe 输出的 "v6.0.1-24-g7fa3f81" 这类后缀。
...
val exactTag = runGit(listOf("describe", "--tags", "--match", "v*", "--match", "PUKEKO_*", "--exact-match"))
val nearestTag = runGit(listOf("describe", "--tags", "--match", "v*", "--match", "PUKEKO_*", "--always"))
