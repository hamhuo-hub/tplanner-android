...
- 常驻 UI 只有三样：**Timeline、底部的 Plan 输入条、右上角的控制器**。底部导航岛已删除。
- 时间轴默认只画今天；没有顶部日期条，也没有右下角的加号。换日在右上角控制器里，
  换完会短暂浮出「9月18日 · 周五」（`DayFlashLabel`）报一下方位；不在今天时控制器自己
  显示一个极小的日期数字。长按空白处仍能在指定时刻落一个任务。
- 不在今天时，底部输入条上方还会浮出一个**回今天的小圆点**（`ReturnToTodayDot`）：
  点一下回到今天，同时报一次日期；回到今天后它自己消失。
- 时间刻度写在左侧窄槽里、数字旋转 90° 自下而上读，横向只占一行字高，宽度来自
  `component.agenda.timeGutterWidth`。
- 底部只有一条 Plan 输入条：整条可点进入那张纸，右侧一个很小的 `▲` 只是"这里可以展开"的
  提示（平时次级文字色，按住/聚焦才变强调色，没有圆形底色、没有橙色大块）。展开后面板
  **完全不透明**：顶部的缝留给 "Plan" 字样与关闭入口，只有上面两个圆角，背后不会透出时间轴。
- **Plan 正文不落盘**：它不是记录，只活在这一次会话里，退出即销毁，也没有草稿与恢复。
  唯一会被写入的是用户在预览里点过「确认添加」的那些日程——没确认就什么都不新建。
  （日记仍可能由其他客户端同步过来，但 `JcalProjection` 只把 VTODO 当任务，
  VJOURNAL 不会再以"未命名"的样子出现在时间轴或收件箱里。）
- 右上角是一列独立的圆形控制（44dp 圆 + 20dp 图标，父布局完全透明）：Inbox / 回到今天、
  上一天、下一天、设置。收起与展开是两套内容，同一时刻只存在一套。
- 纸上的 ✓ 不是"保存"，而是**把这段自然语言交给 TPlanner 整理**：此时还没有任何日程被创建。
  写 → 交给 TPlanner → 检查理解 → 落入时间，是一条闭环：
  `Collapsed → Editing → Submitting → Preview → Committing → Collapsed`。
- 交给 TPlanner 的那一刻纸会轻微收拢（1.00 → 0.985）、光标消失、底部对勾变成很小的进行状态、
  纸面写「正在整理…」；模型回答后结果长在**同一张纸**上，不是"进入 AI 页面"。
- **预览保留**（AI 不是确定性解析器）：但它只回答"TPlanner 理解得对不对"——没有输入框、
  没有"提取"按钮、没有面向用户的诊断信息。每项只显示三个层级：明确时间 / 「预计」（点开才看
  依据）/ 「未排期」。
- 确认添加后先让新卡片亮起（0.96 → 1.0）再把纸收回去：因果是"刚才那句话被塞进了时间线"。
  失败或没读到待办都退回还能写的纸，原文一字不动；本地兜底、assumptions、地点只进 Logcat。
...
`component.agenda` / `component.plan`，颜色仍取 `semantic.color.*`。
